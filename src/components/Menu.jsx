...
const Menu = ({cat}) => {
  ...
  useEffect(()=>{
    const fetchData = async ()=>{
      try{
        const res = await axios.get(`https://post-gallery-mdsalim.onrender.com/api/posts/?cat=${cat}`)
        setPosts(res.data);
      } catch(error){
        console.log(error);
      }
    ...
    <div className="menu">
        ...
        {posts.map((post)=>(
            <div className="post" key={post.id}>
                <img src={post.img && `https://post-gallery-mdsalim.onrender.com/${post.img}`} alt="" />
                <h2>{post.title}</h2>
